...
function voyage_continue_reading_link() {
	...
	if ( empty( $readmore ) )
		$link = ' <a class="more-link" href="'. get_permalink() . '">' . __( 'read more', 'voyage' ) . '<span class="meta-nav"></span></a>';
	else
		$link = '';
	return $link;			
}
...
function voyage_widgets_init() {
	...
	register_widget( 'Voyage_Social' );
	register_widget( 'Voyage_Marketing' );
	...
	// Full Sidebar 
	register_sidebar( array(
		'name' => __( 'Blog Widget Area (Full)', 'voyage' ),
		'id' => 'full-widget-area',
		'description' => __( 'Available for Left or Right sidebar layout.', 'voyage' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h4 class="widget-title">',
		'after_title' => '</h4>',
	...
	register_sidebar( array(
		'name' => __( 'Home Widget Area 5', 'voyage' ),
		'id' => 'fifth-home-widget-area',
		'description' => __( 'Home Widget Area 5', 'voyage' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h4 class="widget-title">',
		'after_title' => '</h4>',
	...
	register_sidebar( array(
		'name' => __( 'bbPress/BuddyPress Widget Area', 'voyage' ),
		'id' => 'bbp-widget-area',
		'description' => __( 'bbPress/BuddyPress Widget Area', 'voyage' ),
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h4 class="widget-title">',
		'after_title' => '</h4>',
...
// Prints Post Date
function voyage_meta_date( $style = 1 ) {
	global $voyage_options, $voyage_entry_meta;

	if ( $voyage_options['showdate'] ) {
		if ( ( 1 == $voyage_options['datestyle'] && 1 == $style ) || 3 == $style ) {
			echo '<i class="icon-calendar meta-summary-icon"></i>';
			printf( __( '<time class="entry-date" datetime="%1$s">%2$s</time>', 'voyage' ),
			esc_attr( get_the_date( 'c' ) ),
			esc_html( get_the_date() ) );				
		}
		elseif ( 2 == $voyage_options['datestyle'] && 2 == $style ) {
...
if ( ! function_exists( 'voyage_comment_prep' ) ) :
...
function voyage_posted_category() {
	global $voyage_options;
	$flag = 1;
	if ( is_single() && 1 == $voyage_options['titlebar'] )
		$flag = 0;
//	if ( 'post' == get_post_type() && ! is_category() ) {
	if ( 'post' == get_post_type() && $flag ) {		
		echo '<span class="entry-meta entry-meta-top">';	
		// Featured
		if ( is_sticky() ) {
			printf( '<span class="entry-featured">%1$s  &bull; </span>', __( 'Featured ', 'voyage') );
		}
   		else {
		...
		if( function_exists( 'the_ratings' ) ) { //WP Post Rating Plugin
			the_ratings();
		}	
		echo '</span>';
	}
...
function voyage_posted_on( $style = 1 ) {
	global $voyage_options;
	
	if ( 2 == $style ) {
		voyage_meta_date( $style );		
	}
	elseif ( 'post' == get_post_type() ) {
		echo '<span class="entry-meta entry-meta-middle">';		

		voyage_meta_date();
		if ( is_single() && 1 == $voyage_options['titlebar'] && ! has_post_format( 'aside' ) ) 
			voyage_meta_category();
		voyage_meta_author();
		if ( $voyage_options['sharesocial'] 
				&& ($voyage_options['share_top'] || $voyage_options['share_sum_top'] ) && ! has_post_format( 'aside' ) )
...
function voyage_post_summary_meta( $meta_flag = 0 ) {
	global $voyage_entry_meta;
	if ( ( $voyage_entry_meta || $meta_flag ) && 'post' == get_post_type() ) {
...
function voyage_body_classes( $classes ) {
	...
	if ( $voyage_options['fixed_menu'] && ( has_nav_menu( 'top-menu' ) || $voyage_options['sociallink_top'] ) )
		$classes[] = 'fixed-top-menu';
	
	if ( 3 == $voyage_options['fp_image'] && ( is_page_template( 'page-templates/featured.php' ) || ( is_home() && 1 == $voyage_options['homepage'] ) ) )
		$classes[] = 'fullscreen';

	if ( is_page_template( 'page-templates/fullscreen.php' ) )
		$classes[] = 'full-screen-page';
...
function voyage_scripts_method() {	
	...
	wp_enqueue_style('bootstrap', $theme_uri . '/css/bootstrap.min.css', false, '2.3.1' );
	wp_enqueue_style('fontawesome', $theme_uri . '/css/font-awesome.min.css', array( 'bootstrap' ), '3.0.2' );	

	$voyage_dep = array( 'bootstrap' );
	if ( $voyage_options['ao_colorbox'] ) {
		wp_enqueue_script( 'colorbox' , $theme_uri . '/js/jquery.colorbox-min.js', array( 'jquery'), '1.3.30', true );	
		$voyage_dep[] = 'colorbox';
	}
	if ( $voyage_options['ao_pprint'] ) {
		wp_enqueue_script( 'prettify' , $theme_uri . '/js/prettify.js', array( 'jquery'), null, true ); 		
		$voyage_dep[] = 'prettify';
	}
    		
	if ( $voyage_options['resp_menu'] )	{
		wp_enqueue_script( 'mobile-menu' , $theme_uri . '/js/jquery.mobilemenu.min.js', array( 'jquery'), null, true );
		$voyage_dep[] = 'mobile-menu';
	}
	wp_enqueue_style( 'voyage', $theme_uri . '/css/voyage-style.min.css', array( 'bootstrap', 'fontawesome' ), VOAYGE_THEME_VERSION );
//	wp_enqueue_style( 'voyage', $theme_uri . '/dev/voyage-style.css', array( 'bootstrap', 'fontawesome' ), VOAYGE_THEME_VERSION );
	$child_pre = array( 'voyage' );
	
	if ( 1 == $voyage_options['mobile_style'] ) {
		echo '<meta name="viewport" content="width=device-width initial-scale=1.0" />' . "\n";
		wp_enqueue_style( 'voyage-responsive', $theme_uri . '/css/responsive.min.css', array( 'voyage'), VOAYGE_THEME_VERSION );
//		wp_enqueue_style( 'voyage-responsive', $theme_uri . '/dev/responsive.css', array( 'voyage'), VOAYGE_THEME_VERSION );
		$child_pre[] = 'voyage-responsive';
	}
	
	if ( $voyage_options['sharesocial'] )
		wp_enqueue_style('voyage-sharing', $theme_uri . '/css/voyage-sharing.css', array( 'sharedaddy' ), VOAYGE_THEME_VERSION );		
	
	if ( is_page_template( 'page-templates/portfolio-dynamic.php' ) ||
		 is_page_template( 'page-templates/atozindex.php' ) ) {
		wp_enqueue_script( 'infinite-scroll' , $theme_uri . '/js/jquery.infinitescroll.min.js', array( 'jquery-masonry' ), '2.0', true );	
		$voyage_dep[] = 'infinite-scroll';
	}
	if ( is_page_template( 'page-templates/imageslider.php' ) ) {
		wp_enqueue_style( 'skitter' , $theme_uri . '/css/skitter.styles.min.css', null, VOAYGE_THEME_VERSION );
//		wp_enqueue_style( 'skitter' , $theme_uri . '/dev/skitter.styles.css', null, VOAYGE_THEME_VERSION );
		$child_pre[] = 'skitter';	
		wp_enqueue_script( 'jquery-easing' , $theme_uri . '/js/jquery.easing.1.3.js', array( 'jquery' ), '1.3', true );	
		wp_enqueue_script( 'skitter' , $theme_uri . '/js/jquery.skitter.min.js', array( 'jquery-easing' ), '4.2.1', true );
		$voyage_dep[] = 'skitter';
	}	
	// Load Scheme CSS
	if ( ! empty( $voyage_options['schemecss'] ) ) {
		wp_enqueue_style('voyage-scheme', $voyage_options['schemecss'], $child_pre, VOAYGE_THEME_VERSION );
		$child_pre[] = 'voyage-scheme';		
	}
	wp_enqueue_script( 'voyage-theme-script', $theme_uri . '/js/voyage.js', $voyage_dep, VOAYGE_THEME_VERSION, true );
	//Load child theme's style.css
    if ( $theme_uri != get_stylesheet_directory_uri() )
		wp_enqueue_style('voyage-child', get_stylesheet_uri(), $child_pre, VOAYGE_THEME_VERSION );
}
if ( ! is_admin() )
