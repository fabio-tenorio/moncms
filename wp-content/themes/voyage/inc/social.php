...
function voyage_social_links() {
	$social_links = array(
		...
		'pinterest' => array(
			'name'  => 'url_pinterest',
			'label' => __( 'Pinterest', 'voyage' ),
		),
		'aboutme' => array(
			'name'  => 'url_aboutme',
			'label' => __( 'About Me', 'voyage' ),
		),
		'stumbleupon' => array(
			'name'  => 'url_stumbleupon',
			'label' => __( 'Stumbleupon', 'voyage' ),
		),
		'rss' => array(
			'name'  => 'url_rss',
...
function voyage_social_connection( $pos = 'top' ) { 
	...
	if ( $flag ) {
		...
		if ( 2 == $voyage_options[$icon_option] )
			$icon_class = "medium-icon";
		elseif ( 3 == $voyage_options[$icon_option] )
...
function voyage_social_post_top() {
	...
	if ( $voyage_options['sharesocial'] && ! has_post_format( 'aside' )
				&&  function_exists( 'sharing_display' ) ) {
		if ( is_single() && $voyage_options['share_top'] )
			echo sharing_display();			
		elseif ( ! is_single() && $voyage_options['share_sum_top'] )
			echo sharing_display();		
	}
}
endif;
...
function voyage_remove_sharing_filters() {
	...
	if ( $voyage_options['sharesocial'] && function_exists( 'sharing_display' ) ) {
