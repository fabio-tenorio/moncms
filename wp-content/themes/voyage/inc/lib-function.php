...
function start_lvl( &$output, $depth = 0, $args = Array() ) {
    ...
    $display_depth = ( $depth + 1 ); // first submenu as 0
    $classes = array(
        'dropdown-menu',
...
if ( ! function_exists( 'voyage_thumbnail_size' ) ) : 
 ...
 */
if ( ! function_exists( 'voyage_get_options' ) ) : 
function voyage_get_options() {
	$options = wp_parse_args( get_option( 'voyage_theme_options' ), voyage_default_theme_options() );
	return $options;
}
endif;
/**
...
function voyage_custom_css() {
	global $voyage_options, $voyage_fonts, $content_width;;
	...
	// Fluid
    if ( '1' == $voyage_options['grid_style'] ) {
	...
	// Inline CSS
    if ( ! empty($voyage_options['voyage_scheme_css'] ) ) {
		echo '<!-- Custom CSS Styles -->' . "\n";
        echo '<style type="text/css" media="screen">' . "\n";
		echo $voyage_options['voyage_scheme_css'] . "\n";
		echo '</style>' . "\n";
	}
...
function voyage_sort_array( &$array, $key ) {
    $sorter = array();
    $ret = array();
    reset( $array );
...
function voyage_comment_form_fields( $arg ) {
	$commenter = wp_get_current_commenter();
	$req = get_option( 'require_name_email' );
	$aria_req = ( $req ? " aria-required='true'" : '' );
		
	$arg['author'] = '<p class="input-prepend comment-form-author"><span class="add-on"><i class="icon-user"></i></span><input id="author" name="author" type="text" placeholder="' . __( 'Name', 'voyage' ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' .  $aria_req . ' /></p>';

	$arg['email'] = '<p class="input-prepend comment-form-email"><span class="add-on"><i class="icon-envelope"></i></span><input id="email" name="email" type="text" placeholder="' . __( 'email', 'voyage' ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30"' .  $aria_req . ' /></p>';
				
    $arg['url'] = '<p class="input-prepend comment-form-url"><span class="add-on"><i class="icon-globe"></i></span><input id="url" name="url" type="text" placeholder="' . __( 'Website', 'voyage' ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p>';
    return $arg;
}
add_filter( 'comment_form_default_fields', 'voyage_comment_form_fields' );

function voyage_comment_form_defaults( $args ) {
	$req = get_option( 'require_name_email' );
	$args['comment_notes_before'] = '<p class="comment-notes">' . ( $req ? __( 'Name and email are required.', 'voyage' ) : '' ) . __( ' Your email address will not be published.', 'voyage' ) . '</p>';
	return $args;
}
add_filter( 'comment_form_defaults', 'voyage_comment_form_defaults' );
